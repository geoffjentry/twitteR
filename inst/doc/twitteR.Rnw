...
The reworking is still underway.  This version brings some bug fixes,
an easier mechanism to convert from the returned object lists to data.frames, 
and support for the Twitter trends API.
...
There are four classes in this package: \Robject{user}, \Robject{status},
\Robject{trend}, and \Robject{directMessage}.  As of this version they have 
...
and allows one to extract that data.  We're able to get the 10 currently
trending topics, see the 20 trending topics per hour for a given day,
or the 30 trending topics per day for a given week.
...
While still not as convenient as it should be, there is a new mechanism to
assist with the process of converting the object lists returned by many
functions in the \Rpackage{twitteR} package into a \Robject{data.frame}
structure.  To do this, every object of every class has a reference
method \Rfunction{toDataFrame} as well as a corresponding S4 method 
\Rfunction{as.data.frame} that works in the traditional sense.  Converting
a single object will typically not be particularly useful by itself, but
it does enable the currently kludgey mechanism for whole list conversion,
such as using the \Robject{publicTweets} variable we had earlier:
...
What we're doing here is calling \Rfunction{lapply} on all of the
\Rclass{status} objects stored in the \Robject{publicTweets} list,
and calling their \Rfunction{as.data.frame} method.  These are combined
by row using \Rfunction{do.call}.
